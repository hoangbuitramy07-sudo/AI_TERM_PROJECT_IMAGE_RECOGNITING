...
keras_model = tf.keras.models.load_model("/Users/thanhha/Documents/AI-final/AI/canteen_cnn_final.keras")
print("Keras Model loaded successfully.")

print("Loading YOLO Model: best.pt...")
yolo_model = YOLO("/Users/thanhha/Documents/AI-final/AI/best.pt")
print("YOLO Model loaded successfully.")

# Định nghĩa các class tương ứng của model Keras
CANONICAL_CLASSES = [
    "com_trang",
    "dau_hu_sot_ca",
    "ca_hu_kho",
...
def predict():
    ...
    # Bảng màu cho 5 bounding box để nhìn chuyên nghiệp hơn
    colors = [
        (255, 165, 0),   # Cam (Rice)
    ...
    for idx, item in enumerate(crops_info):
        ...
        # Tiền xử lý ảnh cho model Keras (kích thước 192x192)
        ...
        # Nhận diện món ăn bằng Keras Model
        ...
        # Nếu món ăn là "Thịt kho trứng", đếm trứng bằng YOLO model
        if dish_class == "thit_kho_trung":
            ...
            # Tính tiền trứng phụ trội (mỗi quả thêm cộng 6.000đ)
            extra_eggs = max(0, egg_count - 1)
            price += extra_eggs * 6000
            note += f", trứng={egg_count}"
            ui_name = f"Thịt kho trứng (x{egg_count} quả)"
            
            # Vẽ thêm các bounding box trứng nhỏ bên trong
            for box in yolo_res[0].boxes:
                bx1, by1, bx2, by2 = map(int, box.xyxy[0])
                # Chuyển đổi tọa độ box YOLO của ảnh crop về tọa độ ảnh gốc
        ...
        # Vẽ bounding box của ngăn thức ăn lên ảnh gốc
        color = colors[idx % len(colors)]
        cv2.rectangle(img_annotated, (x1, y1), (x2, y2), color, 3)
        
    # Encode ảnh kết quả sang base64 để gửi về UI
